...
async function loadManicureData() {
  try {
    const response = await fetch(
      "http://localhost/api/get-data.php?category_id=4",
    );
    const data = await response.json();

    if (Array.isArray(data)) {
      manicureData = data;
      renderCards(manicureData);
    } else {
...
async function getBookedSlotsFromDB(masterId) {
  try {
    ...
    console.error("Помилка отримання зайнятих слотів:", e);
...
function getAvailableSlots(master) {
  ...
  const durationMinutes = parseInt(master.duration) || 60;
  const start = parseInt(master.workHoursStart);
  const end = parseInt(master.workHoursEnd);

  const bookedSlots = getBookedSlotsFromDB(master.id);
...
function filterAndSort() {
  ...
  let filtered = manicureData.filter((master) => {
...
async function bookMaster(masterId, serviceId, date, time) {
  ...
  if (!userData) {
    alert(
      "Please log in to book an appointment! We will redirect you to the login page.",
    );
  ...
  try {
    ...
    if (result.message) {
      alert("Great! You did it! " + result.message);
      location.reload();
    } else {
