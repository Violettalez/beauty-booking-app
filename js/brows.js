...
async function loadBrowsData() {
  try {
    const response = await fetch(
      "http://localhost/api/get-data.php?category_id=1",
    );
    const data = await response.json();

    if (Array.isArray(data)) {
      browsData = data;
      renderCards(browsData);
    } else {
    ...
    if (container)
      container.innerHTML =
        "<p>Помилка завантаження даних. Перевірте з'єднання з БД.</p>";
  }
}
...
async function getAvailableSlotsWithBooked(master, bookedSlots) {
...
function filterAndSort() {
  ...
  let filtered = browsData.filter((master) => {
...
async function renderCards(data) {
  ...
  for (const master of data) {
    ...
    const slots = await getAvailableSlotsWithBooked(master, bookedSlots);
