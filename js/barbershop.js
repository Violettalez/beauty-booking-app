...
async function loadBarbershopData() {
  try {
    const response = await fetch(
      "http://localhost/api/get-data.php?category_id=2",
    );
    const data = await response.json();

    if (Array.isArray(data)) {
      barbershopData = data;
      renderCards(barbershopData);
    } else {
    ...
    if (container) container.innerHTML = "<p>Error: Failed to load data.</p>";
  }
}
...
function filterAndSort() {
  ...
  let filtered = barbershopData.filter((master) => {
